...
public class ProductsController extends BaseCustomController {
	...
	public String showProductDetails(HttpServletRequest request, ModelMap modelMap, 
		...
		return "products/product_details";//show jsp
	}
	...
	public String searchProducts(HttpServletRequest request, ModelMap modelMap) {
		...
		try {
			if(this.getSessionObject(request, GenericConstants.KEY_LIST_ITEMS)==null){
				//get categories once for this session
		...
		modelMap.put("CategoryDTO", new CategoryDTO()); //set ModelAttribute
		return "products/search_products";//show jsp
	}	
	
	@RequestMapping(value = "searchProducts.do", method=RequestMethod.POST)
	public String showSearchProductsResults(HttpServletRequest request, ModelMap modelMap, 
			@ModelAttribute("CategoryDTO") final CategoryDTO categoryDTO,
			@RequestParam(value="submitted",required=true) final Boolean formSubmitted) {
		if(!this.isLoggedIn(request)){//Verify if user is authenticated
			return "redirect:/authentication/notLoggedIn.do";
		}	
		...
		modelMap.put("CategoryDTO", categoryDTO); //set ModelAttribute
		modelMap.put("formWasSubmitted", formSubmitted); //set ModelAttribute
		return "products/search_products";//show jsp
	}
	...
	public String getAllProducts(HttpServletRequest request, ModelMap modelMap,
		...
		//Create ModelAttribute for FORM to be used in searchProducts method
	...
	public String searchProducts(HttpServletRequest request, ModelMap modelMap,
			@ModelAttribute("ProductDTO") final ProductDTO productDTO,
			@RequestParam("idBrand") final Integer brand,
			@RequestParam("idCategory") final Integer category){
		if(!this.isLoggedIn(request)){//Verify if user is authenticated
			return "redirect:/authentication/notLoggedIn.do";
		}
	...
	public String showProductDetailsForUpdate(HttpServletRequest request, ModelMap modelMap,
			@RequestParam(value="idProduct") final Integer idProduct){

		//Call existing logic
		this.showProductDetails(request, modelMap, idProduct);
				
		//Create ModelAttribute for FORM
	...
	public String saveUpdateProduct(HttpServletRequest request, ModelMap modelMap,
			@ModelAttribute("ProductDTO") final ProductDTO productDTO,
			@RequestParam("productImage") final MultipartFile productImage,
			@RequestParam("brand") final Integer brand,
